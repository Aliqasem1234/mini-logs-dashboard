...
from fetch_logs import hamta_loggar, DB_CONFIG
...
# ─────────────────────────────────────────────
# Val av läge: Logga in eller Registrera konto
# ─────────────────────────────────────────────
mode = st.radio("Välj", ["Logga in", "Registrera"], horizontal=True)

# ─────────────────────────────────────────────
# Registreringsvy – skapa nytt användarkonto
# ─────────────────────────────────────────────
if mode == "Registrera":
...
# ─────────────────────────────────────────────
# Formulär för att skapa en ny loggpost
# ─────────────────────────────────────────────
...
if df.empty:
    ...
    # Sammanfattning per loggnivå
    st.subheader("Sammanfattning")
    st.bar_chart(df['level'].value_counts())

    # Tidslinje för senaste 24 timmarna
    st.subheader("Tidslinje (senaste 24h)")
    df['timestamp'] = pd.to_datetime(df['timestamp'])
    last_24h = df[df['timestamp'] >= pd.Timestamp.now() - pd.Timedelta(hours=24)]
    timeline = last_24h.groupby(
        [last_24h['timestamp'].dt.floor('H'), 'level']
